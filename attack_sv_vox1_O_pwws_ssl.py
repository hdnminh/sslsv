#!/usr/bin/env python3
"""
SSL Speech Verification Attack Script with TextAttack Framework
Performs adversarial attacks on SSL-based speech verification systems using text perturbations + TTS
"""
...
def setup_logging(log_dir, tts, attacker_name, seed_value):
    ...
    logger.add(
        os.path.join(log_dir, f"ssl_attack_{tts}_attacker_{attacker_name}_seed_{seed_value}.log"),
...
class SSLSpeechVerificationModel:
    ...
    def compute_eer_threshold(self, method: str = "discrete") -> float:
...
class SSLSpeechVerificationPipelineWrapper(ModelWrapper):
    """Wrapper for SSL speech verification pipeline to work with TextAttack"""

    def __init__(
        self, 
        ssl_sv_model: SSLSpeechVerificationModel, 
        tts: TTSGenerator, 
        ...
        self.ssl_sv_model = ssl_sv_model
        self.tts = tts
    ...
    def _load_reference_embedding(self) -> torch.Tensor:
        ...
        try:
            # Load reference audio - handle both absolute and relative paths
            if os.path.isabs(self.reference_audio_path):
                ref_audio_abs = self.reference_audio_path
            else:
                ref_audio_abs = os.path.join(self.voxceleb_root, self.reference_audio_path)
            
            audio, sr = librosa.load(ref_audio_abs, sr=16000)
            audio_tensor = torch.tensor(audio, dtype=torch.float32)
            
            # Extract embedding using SSL model
            embedding = self.ssl_sv_model.extract_embedding(audio_tensor)
            if embedding is None:
                raise ValueError("Failed to extract reference embedding")
            
            logger.info(f"Reference embedding loaded from {ref_audio_abs}")
            return embedding
    ...
    @property
    def model(self):
        # Return self or the underlying SSL SV model, as needed for TextAttack compatibility
        return self.ssl_sv_model

    def __call__(self, text_inputs):
        """
        Main inference method for TextAttack compatibility with SSL models.
        ...
        for idx, (transcript, audio_file) in enumerate(zip(text_inputs, audio_files)):
            ...
            # Extract embedding and compute similarity using SSL model
            embedding = self.ssl_sv_model.extract_embedding(audio_tensor)
            if embedding is None:
                outputs.append([0.5, 0.5])
                continue
                
            similarity = self.ssl_sv_model.compute_similarity(self.reference_embedding, embedding)
...
def save_partial_results(
    ...
    data = {
        "ssl_model_config": ssl_sv_model.config_path,
        "ssl_model_checkpoint": ssl_sv_model.checkpoint_name,
...
def format_ssl_model_name(ssl_sv_model):
    """Format SSL model name consistently"""
    config_name = os.path.basename(ssl_sv_model.config_path).replace('.yml', '').replace('config', '')
    checkpoint_name = ssl_sv_model.checkpoint_name.replace('.pt', '')
    return f"ssl-{config_name}-{checkpoint_name}"
...
def initialize_ssl_models(args):
    """Initialize SSL speech verification and TTS models"""
...
def prepare_valid_entries(voxceleb_data, args):
    """Filter and prepare valid dataset entries for SSL framework"""
    valid_entries = []
    for idx, entry in enumerate(voxceleb_data):
        ...
        # For SSL framework, use the voxceleb_root to construct absolute paths
        ref_audio_abs = os.path.join(args.voxceleb_root, ref_audio_rel)
        target_audio_abs = os.path.join(args.voxceleb_root, target_audio_rel)
...
def setup_attack_for_entry(entry, ssl_sv_model, tts, AttackerMethod, args):
    """Set up attack components for a single entry"""
    # Create model wrapper for this specific entry
    model_wrapper = SSLSpeechVerificationPipelineWrapper(
        ssl_sv_model=ssl_sv_model,
        tts=tts,
        reference_audio_path=entry['ref_audio_rel'],  # Use relative path for SSL framework
...
def process_single_entry(entry, entry_idx, ssl_sv_model, tts, AttackerMethod, args, main_attacker):
    ...
    if attack_results and len(attack_results) > 0:
        ...
        item = {
            ...
            "reference_audio_path": entry['ref_audio_rel'],
        }
    else:
...
def process_batch(
    ...
    for entry_idx, entry in enumerate(batch_entries):
        item, tts_count = process_single_entry(entry, entry_idx, ssl_sv_model, tts, AttackerMethod, args, main_attacker)
        if item:
...
def run_attacker_experiment(attacker_name, AttackerMethod, valid_entries, ssl_sv_model, tts, args):
    """Run complete experiment for one attacker method"""
    
    # Set up output file
    ssl_model_name_str = format_ssl_model_name(ssl_sv_model).replace('-', '_')
    file_name = os.path.join(args.output_dir, f"{attacker_name}_ssl_vox1-O_{ssl_model_name_str}_{args.seed_value}.json")
    ...
    logger.info(f"Running {attacker_name} SSL attack on all pairs with batch size {BATCH_SIZE}")
    ...
    for batch_idx in range(total_batches):
        ...
        batch_items, batch_tts_count, batch_start_time = process_batch(
            batch_entries, batch_idx, total_batches, ssl_sv_model, tts, AttackerMethod, args, main_attacker
        )
        ...
        save_partial_results(
            batch_items, attacker_name, ssl_sv_model, tts, 
    ...
    if all_consolidated_results:
        ...
        save_final_consolidated_results(
            all_consolidated_results, attacker_name, ssl_sv_model, tts, 
        ...
        logger.info(f"SSL Attack {attacker_name} completed. Runtime: {final_time_cost}")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='SSL Speech Verification Attack Script with TextAttack')
    ...
    parser.add_argument("--target_dataset_path", type=str, default='sv_ssl_attack_results_05072025_16khz/filter_results/all-filtered-cases_label_1_0.6.json')
    parser.add_argument('--output_dir', type=str, default='ssl_attack_results',
                       help='Output directory for analysis json results')
    ...
    parser.add_argument('--batch_size', type=int, default=50,
                       help='Batch size for processing samples (default: 50)')
    parser.add_argument('--similarity_threshold', type=float, default=0.7,
                       help='Similarity threshold for attack (default: 0.7)')
...
if __name__ == "__main__":
    ...
    logger.info(f"=== SSL Speech Verification Attack Experiment Started ===")
    logger.info(f"Start time: {experiment_start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"Output directory: {args.output_dir}")
    logger.info(f"Cache directory: {args.cache_dir}")
    
    # Initialize SSL models
    ssl_sv_model, tts = initialize_ssl_models(args)
    # Load dataset
    ...
    logger.info(f"Starting SSL speech verification attacks (per transcript/ref_audio pair)...")
    logger.info(f"SSL Model Config: {args.ssl_config}")
    logger.info(f"SSL Model Checkpoint: {args.ssl_checkpoint}")
    logger.info(f"EER Method: {args.eer_method}")
    logger.info(f"TTS Model: {args.tts}")
    logger.info(f"Cache Dir: {args.cache_dir}")
    logger.info(f"Output Dir: {args.output_dir}")
    logger.info(f"VoxCeleb Root: {args.voxceleb_root}")
    ...
    for attacker_name, AttackerMethod in ATTACKER_MAP.items():
        run_attacker_experiment(attacker_name, AttackerMethod, valid_entries, ssl_sv_model, tts, args)
    ...
    logger.info("=== All SSL speech verification attacks completed! ===")
